...
![Gráfica Resultado Modelos](reports/grafica_tijera.png)
...
La **Serie A** muestra una dinámica de aprendizaje compleja: el $Bias^2$ es el factor dominante en los modelos iniciales y decrece conforme aumenta la complejidad, mientras que la **Varianza** presenta un comportamiento inestable. Es notable el caso del modelo **A1**, donde se observa una **varianza negativa (-0.021)**. Este fenómeno indica un underfitting tan severo que el modelo no logra aprender patrones consistentes en el entrenamiento, resultando en un rendimiento de test ligeramente superior por fluctuación estadística. A partir de A3, la varianza se hace positiva y oscilante, evidenciando las dificultades de las CNNs desde cero para generalizar con arquitecturas intermedias.

Por el contrario, la **Serie D** rompe este patrón gracias al preentrenamiento en ImageNet, manteniendo niveles de $Bias^2$ y Varianza extremadamente bajos y estables en toda la gama. Como modelo definitivo, seleccionamos **D4-Minimal**. Es crucial destacar que en toda la Serie D se aplicó un **full fine-tuning**, descongelando el backbone completo con un learning rate muy bajo para preservar el conocimiento previo.

La superioridad de **D4-Minimal** radica en su arquitectura optimizada: posee una cabeza de clasificación podada al mínimo (sin capas densas ocultas, actuando directamente tras el Dropout). Pese a esta simplicidad, D4 no solo iguala, sino que **supera** al modelo de máxima complejidad (**D0-Full**), registrando un error total de apenas **0.071 frente al 0.086** de D0. Esto demuestra que el extractor de características de EfficientNet es tan eficiente que la adición de capas densas en la cabeza (como en D0 o D1) resulta contraproducente, induciendo una mayor varianza y limitando la capacidad de generalización. Con un **F1-macro de 0.929 en test**, D4 se consolida como la solución más robusta y eficiente del experimento.
